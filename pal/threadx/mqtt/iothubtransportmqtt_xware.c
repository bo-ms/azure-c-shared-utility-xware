...
#include <stdlib.h>
#include "iothubtransportmqtt_xware.h"
...
static TRANSPORT_LL_HANDLE IoTHubTransportMqtt_Create(const IOTHUBTRANSPORT_CONFIG* config, TRANSPORT_CALLBACKS_INFO* cb_info, void* ctx)
{
    ...
    return IoTHubTransport_MQTT_Xware_Create(config, getIoTransportProvider, cb_info, ctx);
}
...
static void IoTHubTransportMqtt_Destroy(TRANSPORT_LL_HANDLE handle)
{
    ...
    IoTHubTransport_MQTT_Xware_Destroy(handle);
}
...
static int IoTHubTransportMqtt_Subscribe(IOTHUB_DEVICE_HANDLE handle)
{
    ...
    return IoTHubTransport_MQTT_Xware_Subscribe(handle);
}
...
static void IoTHubTransportMqtt_Unsubscribe(IOTHUB_DEVICE_HANDLE handle)
{
    ...
    IoTHubTransport_MQTT_Xware_Unsubscribe(handle);
}
...
{
    return IoTHubTransport_MQTT_Xware_Subscribe_DeviceMethod(handle);
}
...
{
    IoTHubTransport_MQTT_Xware_Unsubscribe_DeviceMethod(handle);
}
...
{
    return IoTHubTransport_MQTT_Xware_Subscribe_DeviceTwin(handle);
}
...
{
    IoTHubTransport_MQTT_Xware_Unsubscribe_DeviceTwin(handle);
}
...
{
    // Codes_SRS_IOTHUB_MQTT_TRANSPORT_09_001: [ IoTHubTransportMqtt_GetTwinAsync shall call into the IoTHubTransport_MQTT_Xware_GetTwinAsync function. ]
    return IoTHubTransport_MQTT_Xware_GetTwinAsync(handle, completionCallback, callbackContext);
}
...
{
    return IoTHubTransport_MQTT_Xware_DeviceMethod_Response(handle, methodId, response, response_size, status_response);
}
...
{
    return IoTHubTransport_MQTT_Xware_SendMessageDisposition(message_data, disposition);
}
...
{
    return IoTHubTransport_MQTT_Xware_ProcessItem(handle, item_type, iothub_item);
}
...
static void IoTHubTransportMqtt_DoWork(TRANSPORT_LL_HANDLE handle)
{
    ...
    IoTHubTransport_MQTT_Xware_DoWork(handle);
}
...
static int IoTHubTransportMqtt_SetRetryPolicy(TRANSPORT_LL_HANDLE handle, IOTHUB_CLIENT_RETRY_POLICY retryPolicy, size_t retryTimeoutLimitInSeconds)
{
    ...
    return IoTHubTransport_MQTT_Xware_SetRetryPolicy(handle, retryPolicy, retryTimeoutLimitInSeconds);
}
...
static IOTHUB_CLIENT_RESULT IoTHubTransportMqtt_GetSendStatus(IOTHUB_DEVICE_HANDLE handle, IOTHUB_CLIENT_STATUS *iotHubClientStatus)
{
    ...
    return IoTHubTransport_MQTT_Xware_GetSendStatus(handle, iotHubClientStatus);
}
...
static IOTHUB_CLIENT_RESULT IoTHubTransportMqtt_SetOption(TRANSPORT_LL_HANDLE handle, const char* option, const void* value)
{
    ...
    return IoTHubTransport_MQTT_Xware_SetOption(handle, option, value);
}
...
static IOTHUB_DEVICE_HANDLE IoTHubTransportMqtt_Register(TRANSPORT_LL_HANDLE handle, const IOTHUB_DEVICE_CONFIG* device, PDLIST_ENTRY waitingToSend)
{
    ...
    return IoTHubTransport_MQTT_Xware_Register(handle, device, waitingToSend);
}
...
static void IoTHubTransportMqtt_Unregister(IOTHUB_DEVICE_HANDLE deviceHandle)
{
    ...
    IoTHubTransport_MQTT_Xware_Unregister(deviceHandle);
}
...
static STRING_HANDLE IoTHubTransportMqtt_GetHostname(TRANSPORT_LL_HANDLE handle)
{
    ...
    return IoTHubTransport_MQTT_Xware_GetHostname(handle);
}

static int IotHubTransportMqtt_Subscribe_InputQueue(IOTHUB_DEVICE_HANDLE handle)
{
    /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_31_14: [ IoTHubTransportMqtt_Subscribe shall subscribe the TRANSPORT_LL_HANDLE by calling into IoTHubTransport_MQTT_Xware_Subscribe_InputQueue. ] */
    return IoTHubTransport_MQTT_Xware_Subscribe_InputQueue(handle);
}

static void IotHubTransportMqtt_Unsubscribe_InputQueue(IOTHUB_DEVICE_HANDLE handle)
{
    /* Codes_SRS_IOTHUBTRANSPORTAMQP_31_015: [IotHubTransportMQTT_Unsubscribe_InputQueue shall unsubscribe by calling into IoTHubTransport_MQTT_Xware_Unsubscribe_InputQueue. ] */
    IoTHubTransport_MQTT_Xware_Unsubscribe_InputQueue(handle);
}

static int IotHubTransportMqtt_SetCallbackContext(TRANSPORT_LL_HANDLE handle, void* ctx)
{
    return IoTHubTransport_MQTT_Xware_SetCallbackContext(handle, ctx);
}
...
extern const TRANSPORT_PROVIDER* XWARE_MQTT_Protocol(void)
{
    return &myfunc;
}
