...
#include "iothubtransport_mqtt_xware.h"
...
static void free_transport_handle_data(MQTTTRANSPORT_HANDLE_DATA* transport_data)
{
    ...
    {
        xware_mqtt_client_deinit(transport_data->mqttClient);
    }
...
int IoTHubTransport_MQTT_Xware_SetRetryPolicy(TRANSPORT_LL_HANDLE handle, IOTHUB_CLIENT_RETRY_POLICY retryPolicy, size_t retryTimeoutLimitInSeconds)
{
    int result;

    if (handle == NULL)
    {
        /* Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_25_041: [**If any handle is NULL then IoTHubTransport_MQTT_Xware_SetRetryPolicy shall return resultant line.] */
    ...
    else
    {
        ...
        // Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_09_006: [ IoTHubTransport_MQTT_Xware_SetRetryPolicy shall set the retry logic by calling retry_control_create() with retry policy and retryTimeout as parameters]
        // Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_09_009: [ If retry_control_create() fails then IoTHubTransport_MQTT_Xware_SetRetryPolicy shall revert to previous retry policy and return non-zero value ]
        ...
        else
        {
            ...
            /*Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_25_045: [**If retry logic for specified parameters of retry policy and retryTimeoutLimitInSeconds is created successfully then IoTHubTransport_MQTT_Xware_SetRetryPolicy shall return 0]*/
            result = 0;
        }
    }
...
static int addSystemPropertiesTouMqttMessage(IOTHUB_MESSAGE_HANDLE iothub_message_handle, STRING_HANDLE topic_string, size_t* index_ptr, bool urlencode)
{
    ...
    /* Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_07_052: [ IoTHubTransport_MQTT_Xware_DoWork shall check for the CorrelationId property and if found add the value as a system property in the format of $.cid=<id> ] */
    ...
    /* Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_07_053: [ IoTHubTransport_MQTT_Xware_DoWork shall check for the MessageId property and if found add the value as a system property in the format of $.mid=<id> ] */
    if (result == 0)
    {
    ...
    // Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_09_010: [ `IoTHubTransport_MQTT_Xware_DoWork` shall check for the ContentType property and if found add the `value` as a system property in the format of `$.ct=<value>` ]
    if (result == 0)
    {
    ...
    // Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_09_011: [ `IoTHubTransport_MQTT_Xware_DoWork` shall check for the ContentEncoding property and if found add the `value` as a system property in the format of `$.ce=<value>` ]
    if (result == 0)
    {
...
static int addDiagnosticPropertiesTouMqttMessage(IOTHUB_MESSAGE_HANDLE iothub_message_handle, STRING_HANDLE topic_string, size_t* index_ptr)
{
    ...
    // Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_09_014: [ `IoTHubTransport_MQTT_Xware_DoWork` shall check for the diagnostic properties including diagid and diagCreationTimeUtc and if found both add them as system property in the format of `$.diagid` and `$.diagctx` respectively]
    ...
    if (diagnosticData != NULL)
    {
        ...
        {
            // Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_09_015: [ `IoTHubTransport_MQTT_Xware_DoWork` shall check whether diagid and diagCreationTimeUtc be present simultaneously, treat as error if not]
...
static STRING_HANDLE addPropertiesTouMqttMessage(IOTHUB_MESSAGE_HANDLE iothub_message_handle, const char* eventTopic, bool urlencode)
{
    ...
    // Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_31_060: [ `IoTHubTransport_MQTT_Xware_DoWork` shall check for the OutputName property and if found add the alue as a system property in the format of $.on=<value> ]
    if (result != NULL)
    {
...
static int publish_mqtt_telemetry_msg(PMQTTTRANSPORT_HANDLE_DATA transport_data, MQTT_MESSAGE_DETAILS_LIST* mqttMsgEntry, const unsigned char* payload, size_t len)
{
    ...
    else
    {
        ...
        {
            if (xware_mqtt_client_publish(transport_data->mqttClient, mqttMsgEntry ->packet_id, (const unsigned char*)STRING_c_str(msgTopic), strlen(STRING_c_str(msgTopic)), payload, len) != 0)
            {
...
static int publish_device_method_message(MQTTTRANSPORT_HANDLE_DATA* transport_data, int status_code, STRING_HANDLE request_id, const unsigned char* response, size_t response_size)
{
    ...
    else
    {
        ...
        if (xware_mqtt_client_publish(transport_data->mqttClient, packet_id, (const unsigned char*)STRING_c_str(msg_topic), strlen(STRING_c_str(msg_topic)), response, response_size) != 0)
        {
...
static int publish_device_twin_get_message(MQTTTRANSPORT_HANDLE_DATA* transport_data, MQTT_DEVICE_TWIN_ITEM* mqtt_info)
{
    ...
    {
		if (xware_mqtt_client_publish(transport_data->mqttClient, mqtt_info->packet_id, (const unsigned char*)STRING_c_str(msg_topic), strlen(STRING_c_str(msg_topic)), NULL, 0) != 0)
		{
			LogError("Failed publishing to mqtt client.");
			free(mqtt_info);
			result = MU_FAILURE;
...
static int publish_device_twin_message(MQTTTRANSPORT_HANDLE_DATA* transport_data, IOTHUB_DEVICE_TWIN* device_twin_info, MQTT_DEVICE_TWIN_ITEM* mqtt_info)
{
    ...
    else
    {
        ...
        {
            if (xware_mqtt_client_publish(transport_data->mqttClient, mqtt_info->packet_id, (const unsigned char*)STRING_c_str(msgTopic), strlen(STRING_c_str(msgTopic)), data_buff->buffer, data_buff->size) != 0)
            {
...
// Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_31_061: [ If the message is sent to an input queue, `IoTHubTransport_MQTT_Xware_DoWork` shall parse out to the input queue name and store it in the message with IoTHubMessage_SetInputName ]
// Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_31_062: [ If IoTHubTransport_MQTT_Xware_DoWork receives a malformatted inputQueue, it shall fail ]
...
static int setMqttMessagePropertyIfPossible(IOTHUB_MESSAGE_HANDLE IoTHubMessage, const char* propName, const char* propValue, size_t nameLen)
{
    ...
    if (nameLen > 4)
    {
        ...
        {
            // Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_31_063: [ If type is IOTHUB_TYPE_TELEMETRY and the system property `$.cdid` is defined, its value shall be set on the IOTHUB_MESSAGE_HANDLE's ConnectionDeviceId property ]
        ...
        {
            // Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_31_064: [ If type is IOTHUB_TYPE_TELEMETRY and the system property `$.cmid` is defined, its value shall be set on the IOTHUB_MESSAGE_HANDLE's ConnectionModuleId property ]
    ...
    if (nameLen > 2)
    {
        // Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_09_012: [ If type is IOTHUB_TYPE_TELEMETRY and the system property `$.ct` is defined, its value shall be set on the IOTHUB_MESSAGE_HANDLE's ContentType property ]
        ...
        // Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_09_013: [ If type is IOTHUB_TYPE_TELEMETRY and the system property `$.ce` is defined, its value shall be set on the IOTHUB_MESSAGE_HANDLE's ContentEncoding property ]
...
static void mqtt_notification_callback(char* topic, size_t topic_length, char* message, size_t message_length, void* callbackCtx)
{
        ...
        else
        {
            ...
            if (type == IOTHUB_TYPE_DEVICE_TWIN)
            {
                ...
                else
                {
                    if (notification_msg)
                    {
                        transportData->transport_callbacks.twin_retrieve_prop_complete_cb(DEVICE_TWIN_UPDATE_PARTIAL, (const unsigned char*)message, message_length, transportData->transport_ctx);
                    }
                    else
                    {
                            ...
                            if (request_id == msg_entry->packet_id)
                            {
                                ...
                                if (msg_entry->device_twin_msg_type == RETRIEVE_PROPERTIES)
                                {
                                    if (msg_entry->userCallback == NULL)
                                    {
                                        ...
                                        transportData->transport_callbacks.twin_retrieve_prop_complete_cb(DEVICE_TWIN_UPDATE_COMPLETE, (const unsigned char*)message, message_length, transportData->transport_ctx);
                                    ...
                                    else
                                    {
                                        ...
                                        msg_entry->userCallback(DEVICE_TWIN_UPDATE_COMPLETE, (const unsigned char*)message, message_length, msg_entry->userContext);
                                    }
                                }
                                else
                                {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (transportData->transport_callbacks.method_complete_cb(STRING_c_str(method_name), (const unsigned char*)message, message_length, (void*)dev_method_info, transportData->transport_ctx) != 0)
                            {
            ...
            {
                IOTHUB_MESSAGE_HANDLE IoTHubMessage = IoTHubMessage_CreateFromByteArray((const unsigned char*)message, message_length);
            ...
            case MQTT_CLIENT_ON_CONNACK:
            {
                ...
                if (connack != NULL)
                {
                    if (connack->returnCode == CONNECTION_ACCEPTED)
                    {
                        ...
                        // Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_09_008: [ Upon successful connection the retry control shall be reset using retry_control_reset() ]
...
static void DisconnectFromClient(PMQTTTRANSPORT_HANDLE_DATA transport_data)
{
    if (transport_data->currPacketState != DISCONNECT_TYPE)
    {
        ...
        (void)xware_mqtt_client_disconnect(transport_data->mqttClient, mqtt_disconnect_cb, &transport_data->disconnect_recv_flag);
...
static void SubscribeToMqttProtocol(PMQTTTRANSPORT_HANDLE_DATA transport_data)
{
    if (transport_data->topics_ToSubscribe != UNSUBSCRIBE_FROM_TOPIC)
    {
        ...
        {
            /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_016: [IoTHubTransport_MQTT_Xware_Subscribe shall call mqtt_client_subscribe to subscribe to the Message Topic.] */
            if (xware_mqtt_client_subscribe(transport_data->mqttClient, packet_id, subscribe, subscribe_count) != 0)
            {
                /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_017: [Upon failure IoTHubTransport_MQTT_Xware_Subscribe shall return a non-zero value.] */
                LogError("Failure: mqtt_client_subscribe returned error.");
            }
            else
            {
                /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_018: [On success IoTHubTransport_MQTT_Xware_Subscribe shall return 0.] */
...
static int GetTransportProviderIfNecessary(PMQTTTRANSPORT_HANDLE_DATA transport_data)
{
    ...
    if (transport_data->xioTransport == NULL)
    {
        ...
        /* Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_01_011: [ If no `proxy_data` option has been set, NULL shall be passed as the argument `mqtt_transport_proxy_options` when calling the function `get_io_transport` passed in `IoTHubTransport_MQTT_Xware__Create`. ]*/
        ...
        /* Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_01_010: [ If the `proxy_data` option has been set, the proxy options shall be filled in the argument `mqtt_transport_proxy_options` when calling the function `get_io_transport` passed in `IoTHubTransport_MQTT_Xware__Create` to obtain the underlying IO handle. ]*/
...
static int SendMqttConnectMsg(PMQTTTRANSPORT_HANDLE_DATA transport_data)
{
    ...
    if (result == 0)
    {
        ...
        {
            if (xware_mqtt_client_connect(transport_data->mqttClient, STRING_c_str(transport_data->hostAddress), STRING_c_str(transport_data->configPassedThroughUsername), sasToken) != 0)
            {
...
static int InitializeConnection(PMQTTTRANSPORT_HANDLE_DATA transport_data)
{
    ...
    if (!transport_data->isDestroyCalled)
    {
        ...
        // Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_09_007: [ IoTHubTransport_MQTT_Xware_DoWork shall try to reconnect according to the current retry policy set ]
        ...
        // Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_09_001: [ IoTHubTransport_MQTT_Xware_DoWork shall trigger reconnection if the mqtt_client_connect does not complete within `keepalive` seconds]
        ...
        else if (transport_data->mqttClientStatus == MQTT_CLIENT_STATUS_CONNECTED)
        {
            ...
            else
            {
                ...
                if (cred_type != IOTHUB_CREDENTIAL_TYPE_X509 && cred_type != IOTHUB_CREDENTIAL_TYPE_X509_ECC)
                {
                    ...
                    {
                        /* Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_07_058: [ If the sas token has timed out IoTHubTransport_MQTT_Xware_DoWork shall disconnect from the mqtt client and destroy the transport information and wait for reconnect. ] */
                        DisconnectFromClient(transport_data);
...
static PMQTTTRANSPORT_HANDLE_DATA InitializeTransportHandleData(const IOTHUB_CLIENT_CONFIG* upperConfig, PDLIST_ENTRY waitingToSend, IOTHUB_AUTHORIZATION_HANDLE auth_module, const char* moduleId)
{
    ...
    else
    {
        ...
        // Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_09_005: [ MQTT transport shall use EXPONENTIAL_WITH_BACK_OFF as default retry policy ]
        ...
        else
        {
            ...
            {
                state->mqttClient = xware_mqtt_client_init(upperConfig, mqtt_notification_callback, mqtt_operation_complete_callback, state, mqtt_error_callback, state);
                ...
                else
                {
                    ...
                    {
                        /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_010: [IoTHubTransport_MQTT_Xware_Create shall allocate memory to save its internal state where all topics, hostname, device_id, device_key, sasTokenSr and client handle shall be saved.] */
...
TRANSPORT_LL_HANDLE IoTHubTransport_MQTT_Xware_Create(const IOTHUBTRANSPORT_CONFIG* config, MQTT_GET_IO_TRANSPORT get_io_transport, TRANSPORT_CALLBACKS_INFO* cb_info, void* ctx)
{
    PMQTTTRANSPORT_HANDLE_DATA result;
    size_t deviceIdSize;

    /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_001: [If parameter config is NULL then IoTHubTransport_MQTT_Xware_Create shall return NULL.] */
    /* Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_07_041: [ if get_io_transport is NULL then IoTHubTransport_MQTT_Xware_Create shall return NULL. ] */
    ...
    /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_002: [If the parameter config's variables upperConfig, auth_module_handle or waitingToSend are NULL then IoTHubTransport_MQTT_Xware_Create shall return NULL.] */
    else if (config->auth_module_handle == NULL)
    {
        LogError("Invalid Argument: auth_module_handle is NULL)");
        result = NULL;
    }
    /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_002: [If the parameter config's variables upperConfig or waitingToSend are NULL then IoTHubTransport_MQTT_Xware_Create shall return NULL.] */
    /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_003: [If the upperConfig's variables deviceId, both deviceKey and deviceSasToken, iotHubName, protocol, or iotHubSuffix are NULL then IoTHubTransport_MQTT_Xware_Create shall return NULL.] */
    /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_03_003: [If both deviceKey & deviceSasToken fields are NOT NULL then IoTHubTransport_MQTT_Xware_Create shall return NULL.] */
    ...
    /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_006: [If the upperConfig's variables deviceId is an empty strings or length is greater then 128 then IoTHubTransport_MQTT_Xware_Create shall return NULL.] */
    ...
    /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_003: [If the upperConfig's variables deviceId, deviceKey, iotHubName, protocol, or iotHubSuffix are NULL then IoTHubTransport_MQTT_Xware_Create shall return NULL.] */
    ...
    /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_009: [If any error is encountered then IoTHubTransport_MQTT_Xware_Create shall return NULL.] */
    /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_011: [On Success IoTHubTransport_MQTT_Xware_Create shall return a non-NULL value.] */
    return result;
}

void IoTHubTransport_MQTT_Xware_Destroy(TRANSPORT_LL_HANDLE handle)
{
    /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_012: [IoTHubTransport_MQTT_Xware_Destroy shall do nothing if parameter handle is NULL.] */
    PMQTTTRANSPORT_HANDLE_DATA transport_data = (PMQTTTRANSPORT_HANDLE_DATA)handle;
    if (transport_data != NULL)
    {
        ...
        /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_014: [IoTHubTransport_MQTT_Xware_Destroy shall free all the resources currently in use.] */
        /* Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_01_012: [ `IoTHubTransport_MQTT_Xware_Destroy` shall free the stored proxy options. ]*/
        free_transport_handle_data(transport_data);
    }
}

int IoTHubTransport_MQTT_Xware_Subscribe_DeviceTwin(IOTHUB_DEVICE_HANDLE handle)
{
    int result;
    PMQTTTRANSPORT_HANDLE_DATA transport_data = (PMQTTTRANSPORT_HANDLE_DATA)handle;
    if (transport_data == NULL)
    {
        /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_042: [If the parameter handle is NULL than IoTHubTransport_MQTT_Xware_Subscribe shall return a non-zero value.] */
        LogError("Invalid handle parameter. NULL.");
        result = MU_FAILURE;
    }
    else
    {
        if (transport_data->topic_GetState == NULL)
        {
            /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_044: [IoTHubTransport_MQTT_Xware_Subscribe_DeviceTwin shall construct the get state topic string and the notify state topic string.] */
            transport_data->topic_GetState = STRING_construct(TOPIC_GET_DESIRED_STATE);
            if (transport_data->topic_GetState == NULL)
            {
                /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_046: [Upon failure IoTHubTransport_MQTT_Xware_Subscribe_DeviceTwin shall return a non-zero value.] */
                LogError("Failure: unable constructing reported state topic");
                result = MU_FAILURE;
            }
            else
            {
                /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_047: [On success IoTHubTransport_MQTT_Xware_Subscribe_DeviceTwin shall return 0.] */
                transport_data->topics_ToSubscribe |= SUBSCRIBE_GET_REPORTED_STATE_TOPIC;
                result = 0;
            }
        }
...
void IoTHubTransport_MQTT_Xware_Unsubscribe_DeviceTwin(IOTHUB_DEVICE_HANDLE handle)
{
    PMQTTTRANSPORT_HANDLE_DATA transport_data = (PMQTTTRANSPORT_HANDLE_DATA)handle;
    /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_048: [If the parameter handle is NULL than IoTHubTransport_MQTT_Xware_Unsubscribe_DeviceTwin shall do nothing.] */
    if (transport_data != NULL)
    {
        if (transport_data->topic_GetState != NULL)
        {
            /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_049: [If subscribe_state is set to IOTHUB_DEVICE_TWIN_DESIRED_STATE then IoTHubTransport_MQTT_Xware_Unsubscribe_DeviceTwin shall unsubscribe from the topic_GetState to the mqtt client.] */
            transport_data->topics_ToSubscribe &= ~SUBSCRIBE_GET_REPORTED_STATE_TOPIC;
            STRING_delete(transport_data->topic_GetState);
        ...
        if (transport_data->topic_NotifyState != NULL)
        {
            /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_050: [If subscribe_state is set to IOTHUB_DEVICE_TWIN_NOTIFICATION_STATE then IoTHubTransport_MQTT_Xware_Unsubscribe_DeviceTwin shall unsubscribe from the topic_NotifyState to the mqtt client.] */
            transport_data->topics_ToSubscribe &= ~SUBSCRIBE_NOTIFICATION_STATE_TOPIC;
            STRING_delete(transport_data->topic_NotifyState);
...
IOTHUB_CLIENT_RESULT IoTHubTransport_MQTT_Xware_GetTwinAsync(IOTHUB_DEVICE_HANDLE handle, IOTHUB_CLIENT_DEVICE_TWIN_CALLBACK completionCallback, void* callbackContext)
{
    IOTHUB_CLIENT_RESULT result;

    // Codes_SRS_IOTHUB_MQTT_TRANSPORT_09_001: [ If `handle` or `completionCallback` are `NULL` than `IoTHubTransport_MQTT_Xware_GetTwinAsync` shall return IOTHUB_CLIENT_INVALID_ARG. ]
    ...
    else
    {
        ...
        if ((mqtt_info = create_device_twin_get_message(transport_data)) == NULL)
        {
            ...
            // Codes_SRS_IOTHUB_MQTT_TRANSPORT_09_003: [ If any failure occurs, IoTHubTransport_MQTT_Xware_GetTwinAsync shall return IOTHUB_CLIENT_ERROR ]
        ...
        else
        {
            ...
            // Codes_SRS_IOTHUB_MQTT_TRANSPORT_09_004: [ If no failure occurs, IoTHubTransport_MQTT_Xware_GetTwinAsync shall return IOTHUB_CLIENT_OK ]
...
int IoTHubTransport_MQTT_Xware_Subscribe_DeviceMethod(IOTHUB_DEVICE_HANDLE handle)
{
    int result;
    PMQTTTRANSPORT_HANDLE_DATA transport_data = (PMQTTTRANSPORT_HANDLE_DATA)handle;
    ...
    if (transport_data == NULL)
    {
        /*Codes_SRS_IOTHUB_MQTT_TRANSPORT_12_001 : [If the parameter handle is NULL than IoTHubTransport_MQTT_Xware_Subscribe_DeviceMethod shall return a non - zero value.]*/
        LogError("Invalid handle parameter. NULL.");
        result = MU_FAILURE;
    }
    else
    {
        if (transport_data->topic_DeviceMethods == NULL)
        {
            /*Codes_SRS_IOTHUB_MQTT_TRANSPORT_12_004 : [IoTHubTransport_MQTT_Xware_Subscribe_DeviceMethod shall construct the DEVICE_METHOD topic string for subscribe.]*/
            transport_data->topic_DeviceMethods = STRING_construct(TOPIC_DEVICE_METHOD_SUBSCRIBE);
            if (transport_data->topic_DeviceMethods == NULL)
            {
                /*Codes_SRS_IOTHUB_MQTT_TRANSPORT_12_006 : [Upon failure IoTHubTransport_MQTT_Xware_Subscribe_DeviceMethod shall return a non - zero value.]*/
                LogError("Failure: unable constructing device method subscribe topic");
                result = MU_FAILURE;
            }
            else
            {
                /*Codes_SRS_IOTHUB_MQTT_TRANSPORT_12_003 : [IoTHubTransport_MQTT_Xware_Subscribe_DeviceMethod shall set the signaling flag for DEVICE_METHOD topic for the receiver's topic list. ]*/
                transport_data->topics_ToSubscribe |= SUBSCRIBE_DEVICE_METHOD_TOPIC;
                result = 0;
            }
        }
        ...
        {
            /*Codes_SRS_IOTHUB_MQTT_TRANSPORT_12_002 : [If the MQTT transport has been previously subscribed to DEVICE_METHOD topic IoTHubTransport_MQTT_Xware_Subscribe_DeviceMethod shall do nothing and return 0.]*/
            result = 0;
        }

        if (result == 0)
        {
            /*Codes_SRS_IOTHUB_MQTT_TRANSPORT_12_005 : [IoTHubTransport_MQTT_Xware_Subscribe_DeviceMethod shall schedule the send of the subscription.]*/
            /*Codes_SRS_IOTHUB_MQTT_TRANSPORT_12_007 : [On success IoTHubTransport_MQTT_Xware_Subscribe_DeviceMethod shall return 0.]*/
            changeStateToSubscribeIfAllowed(transport_data);
        }
    }
    return result;
...
void IoTHubTransport_MQTT_Xware_Unsubscribe_DeviceMethod(IOTHUB_DEVICE_HANDLE handle)
{
    PMQTTTRANSPORT_HANDLE_DATA transport_data = (PMQTTTRANSPORT_HANDLE_DATA)handle;
    /*Codes_SRS_IOTHUB_MQTT_TRANSPORT_12_008 : [If the parameter handle is NULL than IoTHubTransport_MQTT_Xware_Unsubscribe_DeviceMethod shall do nothing and return.]*/
    if (transport_data != NULL)
    {
        /*Codes_SRS_IOTHUB_MQTT_TRANSPORT_12_009 : [If the MQTT transport has not been subscribed to DEVICE_METHOD topic IoTHubTransport_MQTT_Xware_Unsubscribe_DeviceMethod shall do nothing and return.]*/
        if (transport_data->topic_DeviceMethods != NULL)
        {
            /*Codes_SRS_IOTHUB_MQTT_TRANSPORT_12_010 : [IoTHubTransport_MQTT_Xware_Unsubscribe_DeviceMethod shall construct the DEVICE_METHOD topic string for unsubscribe.]*/
            const char* unsubscribe[1];
            unsubscribe[0] = STRING_c_str(transport_data->topic_DeviceMethods);

            /*Codes_SRS_IOTHUB_MQTT_TRANSPORT_12_011 : [IoTHubTransport_MQTT_Xware_Unsubscribe_DeviceMethod shall send the unsubscribe.]*/
            if (xware_mqtt_client_unsubscribe(transport_data->mqttClient, get_next_packet_id(transport_data), unsubscribe, 1) != 0)
            {
                LogError("Failure calling mqtt_client_unsubscribe");
            }

            /*Codes_SRS_IOTHUB_MQTT_TRANSPORT_12_012 : [IoTHubTransport_MQTT_Xware_Unsubscribe_DeviceMethod shall removes the signaling flag for DEVICE_METHOD topic from the receiver's topic list. ]*/
            STRING_delete(transport_data->topic_DeviceMethods);
...
int IoTHubTransport_MQTT_Xware_DeviceMethod_Response(IOTHUB_DEVICE_HANDLE handle, METHOD_HANDLE methodId, const unsigned char* response, size_t respSize, int status)
{
    int result;
    MQTTTRANSPORT_HANDLE_DATA* transport_data = (MQTTTRANSPORT_HANDLE_DATA*)handle;
    if (transport_data != NULL)
    {
        /* Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_07_042: [ IoTHubTransport_MQTT_Xware_DeviceMethod_Response shall publish an mqtt message for the device method response. ] */
        ...
        else
        {
            ...
            {
                /* Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_07_051: [ If any error is encountered, IoTHubTransport_MQTT_Xware_DeviceMethod_Response shall return a non-zero value. ] */
    ...
    {
        /* Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_07_041: [ If the parameter handle is NULL than IoTHubTransport_MQTT_Xware_DeviceMethod_Response shall return a non-zero value. ] */
        result = MU_FAILURE;
...
int IoTHubTransport_MQTT_Xware_Subscribe(IOTHUB_DEVICE_HANDLE handle)
{
    int result;
    PMQTTTRANSPORT_HANDLE_DATA transport_data = (PMQTTTRANSPORT_HANDLE_DATA)handle;
    if (transport_data == NULL)
    {
        /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_015: [If parameter handle is NULL than IoTHubTransport_MQTT_Xware_Subscribe shall return a non-zero value.] */
        LogError("Invalid handle parameter. NULL.");
        result = MU_FAILURE;
    }
    else
    {
        /* Code_SRS_IOTHUB_MQTT_TRANSPORT_07_016: [IoTHubTransport_MQTT_Xware_Subscribe shall set a flag to enable mqtt_client_subscribe to be called to subscribe to the Message Topic.] */
        ...
        {
            transport_data->topics_ToSubscribe |= SUBSCRIBE_TELEMETRY_TOPIC;
            /* Code_SRS_IOTHUB_MQTT_TRANSPORT_07_035: [If current packet state is not CONNACT, DISCONNECT_TYPE, or PACKET_TYPE_ERROR then IoTHubTransport_MQTT_Xware_Subscribe shall set the packet state to SUBSCRIBE_TYPE.]*/
            changeStateToSubscribeIfAllowed(transport_data);
            result = 0;
        }
    }
...
void IoTHubTransport_MQTT_Xware_Unsubscribe(IOTHUB_DEVICE_HANDLE handle)
{
    PMQTTTRANSPORT_HANDLE_DATA transport_data = (PMQTTTRANSPORT_HANDLE_DATA)handle;
    /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_019: [If parameter handle is NULL then IoTHubTransport_MQTT_Xware_Unsubscribe shall do nothing.] */
    if (transport_data != NULL)
    {
        /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_020: [IoTHubTransport_MQTT_Xware_Unsubscribe shall call mqtt_client_unsubscribe to unsubscribe the mqtt message topic.] */
        const char* unsubscribe[1];
...
IOTHUB_PROCESS_ITEM_RESULT IoTHubTransport_MQTT_Xware_ProcessItem(TRANSPORT_LL_HANDLE handle, IOTHUB_IDENTITY_TYPE item_type, IOTHUB_IDENTITY_INFO* iothub_item)
{
    IOTHUB_PROCESS_ITEM_RESULT result;
    /* Codes_SRS_IOTHUBCLIENT_LL_07_001: [ If handle or iothub_item are NULL then IoTHubTransport_MQTT_Xware_ProcessItem shall return IOTHUB_PROCESS_ERROR. ]*/
    ...
    else
    {
        ...
        if (transport_data->currPacketState == PUBLISH_TYPE)
        {
            ...
            if (item_type == IOTHUB_TYPE_DEVICE_TWIN && transport_data->twin_resp_sub_recv)
            {
                ...
                if (mqtt_info == NULL)
                {
                    /* Codes_SRS_IOTHUBCLIENT_LL_07_004: [ If any errors are encountered IoTHubTransport_MQTT_Xware_ProcessItem shall return IOTHUB_PROCESS_ERROR. ]*/
                    result = IOTHUB_PROCESS_ERROR;
                }
                else
                {
                    /*Codes_SRS_IOTHUBCLIENT_LL_07_003: [ IoTHubTransport_MQTT_Xware_ProcessItem shall publish a message to the mqtt protocol with the message topic for the message type.]*/
                    ...
                    /* Codes_SRS_IOTHUBCLIENT_LL_07_005: [ If successful IoTHubTransport_MQTT_Xware_ProcessItem shall add mqtt info structure acknowledgement queue. ] */
                    DList_InsertTailList(&transport_data->ack_waiting_queue, &mqtt_info->entry);
            ...
            {
                /* Codes_SRS_IOTHUBCLIENT_LL_07_006: [ If the item_type is not a supported type IoTHubTransport_MQTT_Xware_ProcessItem shall return IOTHUB_PROCESS_CONTINUE. ]*/
                result = IOTHUB_PROCESS_CONTINUE;
            }
        }
        else
        {
            /* Codes_SRS_IOTHUBCLIENT_LL_07_002: [ If the mqtt is not ready to publish messages IoTHubTransport_MQTT_Xware_ProcessItem shall return IOTHUB_PROCESS_NOT_CONNECTED. ] */
...
/* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_054: [ IoTHubTransport_MQTT_Xware_DoWork shall subscribe to the Notification and get_state Topics if they are defined. ] */
void IoTHubTransport_MQTT_Xware_DoWork(TRANSPORT_LL_HANDLE handle)
{
    /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_026: [IoTHubTransport_MQTT_Xware_DoWork shall do nothing if parameter handle and/or iotHubClientHandle is NULL.] */
    PMQTTTRANSPORT_HANDLE_DATA transport_data = (PMQTTTRANSPORT_HANDLE_DATA)handle;
    if (transport_data != NULL)
    {
        if (InitializeConnection(transport_data) == 0)
        {
            if (transport_data->mqttClientStatus == MQTT_CLIENT_STATUS_PENDING_CLOSE)
            {
                xware_mqtt_client_disconnect(transport_data->mqttClient, NULL, NULL);
                transport_data->mqttClientStatus = MQTT_CLIENT_STATUS_NOT_CONNECTED;
            }
            ...
            else if (transport_data->currPacketState == SUBACK_TYPE)
            {
                ...
                {
                    /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_055: [ IoTHubTransport_MQTT_Xware_DoWork shall send a device twin get property message upon successfully retrieving a SUBACK on device twin topics. ] */
                    MQTT_DEVICE_TWIN_ITEM* mqtt_info;
            ...
            else if (transport_data->currPacketState == PUBLISH_TYPE)
            {
                ...
                /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_027: [IoTHubTransport_MQTT_Xware_DoWork shall inspect the "waitingToSend" DLIST passed in config structure.] */
                while (currentListEntry != transport_data->waitingToSend)
                {
                    ...
                    {
                        /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_029: [IoTHubTransport_MQTT_Xware_DoWork shall create a MQTT_MESSAGE_HANDLE and pass this to a call to mqtt_client_publish.] */
            ...
            /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_030: [IoTHubTransport_MQTT_Xware_DoWork shall call xware_mqtt_client_dowork everytime it is called if it is connected.] */
            if (transport_data->mqttClientStatus == MQTT_CLIENT_STATUS_CONNECTED)
                xware_mqtt_client_dowork(transport_data->mqttClient);
        }
...
IOTHUB_CLIENT_RESULT IoTHubTransport_MQTT_Xware_GetSendStatus(IOTHUB_DEVICE_HANDLE handle, IOTHUB_CLIENT_STATUS *iotHubClientStatus)
{
    IOTHUB_CLIENT_RESULT result;

    if (handle == NULL || iotHubClientStatus == NULL)
    {
        /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_023: [IoTHubTransport_MQTT_Xware_GetSendStatus shall return IOTHUB_CLIENT_INVALID_ARG if called with NULL parameter.] */
    ...
    else
    {
        ...
        {
            /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_025: [IoTHubTransport_MQTT_Xware_GetSendStatus shall return IOTHUB_CLIENT_OK and status IOTHUB_CLIENT_SEND_STATUS_BUSY if there are currently event items to be sent or being sent.] */
            *iotHubClientStatus = IOTHUB_CLIENT_SEND_STATUS_BUSY;
        }
        else
        {
            /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_024: [IoTHubTransport_MQTT_Xware_GetSendStatus shall return IOTHUB_CLIENT_OK and status IOTHUB_CLIENT_SEND_STATUS_IDLE if there are currently no event items to be sent or being sent.] */
...
IOTHUB_CLIENT_RESULT IoTHubTransport_MQTT_Xware_SetOption(TRANSPORT_LL_HANDLE handle, const char* option, const void* value)
{
    /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_021: [If any parameter is NULL then IoTHubTransport_MQTT_Xware_SetOption shall return IOTHUB_CLIENT_INVALID_ARG.] */
    IOTHUB_CLIENT_RESULT result;
    if (
        (handle == NULL) ||
        (option == NULL) ||
    ...
    {
        result = IOTHUB_CLIENT_INVALID_ARG;
        LogError("invalid parameter (NULL) passed to IoTHubTransport_MQTT_Xware_SetOption.");
    }
    else
    {
        MQTTTRANSPORT_HANDLE_DATA* transport_data = (MQTTTRANSPORT_HANDLE_DATA*)handle;
        ...
        if (strcmp(OPTION_LOG_TRACE, option) == 0)
        {
            ...
            xware_mqtt_client_set_trace(transport_data->mqttClient, transport_data->log_trace, transport_data->raw_trace);
            result = IOTHUB_CLIENT_OK;
        }
        ...
        else if (strcmp(OPTION_KEEP_ALIVE, option) == 0)
        {
            ...
            /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_037 : [If the option parameter is set to supplied int_ptr keepalive is the same value as the existing keepalive then IoTHubTransport_MQTT_Xware_SetOption shall do nothing.] */
            if (*keepAliveOption != transport_data->keepAliveValue)
            {
                ...
                {
                    /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_038: [If the client is connected when the keepalive is set then IoTHubTransport_MQTT_Xware_SetOption shall disconnect and reconnect with the specified keepalive value.] */
                    DisconnectFromClient(transport_data);
                }
            }
            result = IOTHUB_CLIENT_OK;
        ...
        {
            /* Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_01_001: [ If `option` is `proxy_data`, `value` shall be used as an `HTTP_PROXY_OPTIONS*`. ]*/
            HTTP_PROXY_OPTIONS* proxy_options = (HTTP_PROXY_OPTIONS*)value;

            if (transport_data->xioTransport != NULL)
            {
                /* Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_01_007: [ If the underlying IO has already been created, then `IoTHubTransport_MQTT_Xware_SetOption` shall fail and return `IOTHUB_CLIENT_ERROR`. ]*/
                LogError("Cannot set proxy option once the underlying IO is created");
                result = IOTHUB_CLIENT_ERROR;
            }
            else if (proxy_options->host_address == NULL)
            {
                /* Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_01_003: [ If `host_address` is NULL, `IoTHubTransport_MQTT_Xware_SetOption` shall fail and return `IOTHUB_CLIENT_INVALID_ARG`. ]*/
                LogError("NULL host_address in proxy options");
                result = IOTHUB_CLIENT_INVALID_ARG;
            }
            /* Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_01_006: [ If only one of `username` and `password` is NULL, `IoTHubTransport_MQTT_Xware_SetOption` shall fail and return `IOTHUB_CLIENT_INVALID_ARG`. ]*/
            ...
            else
            {
                ...
                /* Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_01_002: [ The fields `host_address`, `port`, `username` and `password` shall be saved for later used (needed when creating the underlying IO to be used by the transport). ]*/
                transport_data->http_proxy_port = proxy_options->port;
                if (mallocAndStrcpy_s(&copied_proxy_hostname, proxy_options->host_address) != 0)
                {
                    /* Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_01_004: [ If copying `host_address`, `username` or `password` fails, `IoTHubTransport_MQTT_Xware_SetOption` shall fail and return `IOTHUB_CLIENT_ERROR`. ]*/
                    LogError("Cannot copy HTTP proxy hostname");
                    result = IOTHUB_CLIENT_ERROR;
                }
                /* Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_01_005: [ `username` and `password` shall be allowed to be NULL. ]*/
                ...
                {
                    /* Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_01_009: [ When setting the proxy options succeeds any previously saved proxy options shall be freed. ]*/
                    free_proxy_data(transport_data);
                    ...
                    /* Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_01_008: [ If setting the `proxy_data` option suceeds, `IoTHubTransport_MQTT_Xware_SetOption` shall return `IOTHUB_CLIENT_OK` ]*/
                    result = IOTHUB_CLIENT_OK;
                }
            }
        }
        else
        {
            ...
            /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_032: [IoTHubTransport_MQTT_Xware_SetOption shall pass down the option to xio_setoption if the option parameter is not a known option string for the MQTT transport.] */
            if (GetTransportProviderIfNecessary(transport_data) == 0)
            {
                ...
                {
                    /* Codes_SRS_IOTHUB_MQTT_TRANSPORT_07_132: [IoTHubTransport_MQTT_Xware_SetOption shall return IOTHUB_CLIENT_INVALID_ARG xio_setoption fails] */
                    result = IOTHUB_CLIENT_INVALID_ARG;
                }
            }
            else
...
TRANSPORT_LL_HANDLE IoTHubTransport_MQTT_Xware_Register(TRANSPORT_LL_HANDLE handle, const IOTHUB_DEVICE_CONFIG* device, PDLIST_ENTRY waitingToSend)
{
    TRANSPORT_LL_HANDLE result = NULL;

    // Codes_SRS_IOTHUB_MQTT_TRANSPORT_17_001: [ IoTHubTransport_MQTT_Xware_Register shall return NULL if the TRANSPORT_LL_HANDLE is NULL.]
    // Codes_SRS_IOTHUB_MQTT_TRANSPORT_17_002: [ IoTHubTransport_MQTT_Xware_Register shall return NULL if device or waitingToSend are NULL.]
    if ((handle == NULL) || (device == NULL) || (waitingToSend == NULL))
    {
        LogError("IoTHubTransport_MQTT_Xware_Register: handle, device or waitingToSend is NULL.");
        result = NULL;
    }
    else
    {
        ...
        // Codes_SRS_IOTHUB_MQTT_TRANSPORT_03_001: [ IoTHubTransport_MQTT_Xware_Register shall return NULL if deviceId, or both deviceKey and deviceSasToken are NULL.]
        if (device->deviceId == NULL)
        {
            LogError("IoTHubTransport_MQTT_Xware_Register: deviceId is NULL.");
            result = NULL;
        }
        // Codes_SRS_IOTHUB_MQTT_TRANSPORT_03_002: [ IoTHubTransport_MQTT_Xware_Register shall return NULL if both deviceKey and deviceSasToken are provided.]
        else if ((device->deviceKey != NULL) && (device->deviceSasToken != NULL))
        {
            LogError("IoTHubTransport_MQTT_Xware_Register: Both deviceKey and deviceSasToken are defined. Only one can be used.");
            result = NULL;
        }
        else
        {
            // Codes_SRS_IOTHUB_MQTT_TRANSPORT_17_003: [ IoTHubTransport_MQTT_Xware_Register shall return NULL if deviceId or deviceKey do not match the deviceId and deviceKey passed in during IoTHubTransport_MQTT_Xware_Create.]
            if (strcmp(STRING_c_str(transport_data->device_id), device->deviceId) != 0)
            {
                LogError("IoTHubTransport_MQTT_Xware_Register: deviceId does not match.");
                result = NULL;
            }
            else if (!check_module_ids_equal(STRING_c_str(transport_data->module_id), device->moduleId))
            {
                LogError("IoTHubTransport_MQTT_Xware_Register: moduleId does not match.");
                result = NULL;
            }
            ...
            {
                LogError("IoTHubTransport_MQTT_Xware_Register: deviceKey does not match.");
                result = NULL;
            }
            else
            {
                ...
                else
                {
                    ...
                    // Codes_SRS_IOTHUB_MQTT_TRANSPORT_17_004: [ IoTHubTransport_MQTT_Xware_Register shall return the TRANSPORT_LL_HANDLE as the TRANSPORT_LL_HANDLE. ]
...
void IoTHubTransport_MQTT_Xware_Unregister(IOTHUB_DEVICE_HANDLE deviceHandle)
{
    // Codes_SRS_IOTHUB_MQTT_TRANSPORT_17_005: [ If deviceHandle is NULL `IoTHubTransport_MQTT_Xware_Unregister` shall do nothing. ]
...
STRING_HANDLE IoTHubTransport_MQTT_Xware_GetHostname(TRANSPORT_LL_HANDLE handle)
{
    STRING_HANDLE result;
    /*Codes_SRS_IOTHUB_MQTT_TRANSPORT_02_001: [ If handle is NULL then IoTHubTransport_MQTT_Xware_GetHostname shall fail and return NULL. ]*/
    if (handle == NULL)
    {
        result = NULL;
    }
    /*Codes_SRS_IOTHUB_MQTT_TRANSPORT_02_002: [ Otherwise IoTHubTransport_MQTT_Xware_GetHostname shall return a non-NULL STRING_HANDLE containg the hostname. ]*/
...
IOTHUB_CLIENT_RESULT IoTHubTransport_MQTT_Xware_SendMessageDisposition(MESSAGE_CALLBACK_INFO* message_data, IOTHUBMESSAGE_DISPOSITION_RESULT disposition)
{
    (void)disposition;
    ...
    if (message_data)
    {
        ...
        {
            /*Codes_SRS_IOTHUB_MQTT_TRANSPORT_10_002: [If any of the messageData fields are NULL, IoTHubTransport_MQTT_Xware_SendMessageDisposition shall fail and return IOTHUB_CLIENT_ERROR. ]*/
    ...
    {
        /*Codes_SRS_IOTHUB_MQTT_TRANSPORT_10_001: [If messageData is NULL, IoTHubTransport_MQTT_Xware_SendMessageDisposition shall fail and return IOTHUB_CLIENT_ERROR. ]*/
...
int IoTHubTransport_MQTT_Xware_Subscribe_InputQueue(IOTHUB_DEVICE_HANDLE handle)
{
    int result;
    PMQTTTRANSPORT_HANDLE_DATA transport_data = (PMQTTTRANSPORT_HANDLE_DATA)handle;
    if (transport_data == NULL)
    {
        // Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_31_066: [ If parameter handle is NULL than IoTHubTransport_MQTT_Xware_Subscribe_InputQueue shall return a non-zero value.]
        LogError("Invalid handle parameter. NULL.");
        result = MU_FAILURE;
    }
    else if (transport_data->module_id == NULL)
    {
        // Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_31_073: [ If module ID is not set on the transpont, IoTHubTransport_MQTT_Xware_Unsubscribe_InputQueue shall fail.]
    ...
    {
        // Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_31_067: [ IoTHubTransport_MQTT_Xware_Subscribe_InputQueue shall set a flag to enable mqtt_client_subscribe to be called to subscribe to the input queue Message Topic.]
        transport_data->topics_ToSubscribe |= SUBSCRIBE_INPUT_QUEUE_TOPIC;
        changeStateToSubscribeIfAllowed(transport_data);
        // Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_31_070: [ On success IoTHubTransport_MQTT_Xware_Subscribe_InputQueue shall return 0.]
        result = 0;
    }
    return result;
}

void IoTHubTransport_MQTT_Xware_Unsubscribe_InputQueue(IOTHUB_DEVICE_HANDLE handle)
{
    PMQTTTRANSPORT_HANDLE_DATA transport_data = (PMQTTTRANSPORT_HANDLE_DATA)handle;
    if ((transport_data != NULL) && (transport_data->topic_InputQueue != NULL))
    {
        // Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_31_072: [ IoTHubTransport_MQTT_Xware_Unsubscribe_InputQueue shall call mqtt_client_unsubscribe to unsubscribe the mqtt input queue message topic.]
        const char* unsubscribe[1];
    ...
    {
        // Codes_SRS_IOTHUB_TRANSPORT_MQTT_XWARE_31_071: [ If parameter handle is NULL then IoTHubTransport_MQTT_Xware_Unsubscribe_InputQueue shall do nothing.]
        LogError("Invalid argument to unsubscribe input queue (NULL).");
    }
}

int IoTHubTransport_MQTT_Xware_SetCallbackContext(TRANSPORT_LL_HANDLE handle, void* ctx)
{
    int result;
    if (handle == NULL)
    {
